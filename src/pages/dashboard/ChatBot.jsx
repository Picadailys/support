...
const ChatBot = () => {
    ...
    const [welcomeMessage, setWelcomeMessage] = useState('Hello, How can I be of help to you today? Kindly select an option that best describes your challenge.'); // State for welcome message
    ...
    return (
        <section className='chat-bot-section-holder xui-pos-relative xui-d-flex'>
            ...
            <div className='chat-bot-left xui-pt-1-half xui-pl-1-half'>
                <div className='xui-h-fluid-100 xui-overflow-y-auto box-shadow-1 xui-pt-2 xui-px-1 xui-pb-7' style={{ borderRadius: '16px 0 0 0' }}>
                    {/* Logo */}
                    <div>
                    ...
                    <div className='chat-bot-input-grp xui-mt-1'>
                        ...
                        <textarea
                            ...
                            style={{ height: '40px', overflow: 'hidden', minHeight: '40px', resize: 'none' }}
                            rows="1"
                            value={welcomeMessage}
                            onChange={(e) => setWelcomeMessage(e.target.value)}
                            onInput={(e) => {
                                e.target.style.height = '40px'; // Reset height
                                e.target.style.height = `${Math.max(e.target.scrollHeight, 40)}px`; // Adjust height
                            }}
                        ></textarea>
                    </div>
                    {/* Theme Selector */}
            ...
            <div className='chat-bot-right xui-pt-1-half xui-pr-1-half'>
                <div className='xui-h-fluid-100 box-shadow-1 xui-py-2 xui-px-1' style={{ borderRadius: '0 16px 0 0' }}>
                    ...
                    <div className='xui-mt-1-half nude-bg-5 xui-p-1 xui-h-350 xui-bdr-rad-half'>
                        <div className={`xui-d-flex ${orientation === 'Left' ? 'xui-flex-dir-row-reverse' : ''}`}>
                            <img className='xui-w-35 xui-h-35 xui-bdr-rad-circle' src={Logo2} alt="" />
                            <div className={`chat-bot-preview xui-bg-white xui-p-1`} style={{ width: 'calc(100% - 35px)' }}>
                                ...
                                ></p>
                                {name && <p className='xui-font-sz-80 xui-font-w-400 xui-line-height-1'>{`- ${name}`}</p>}
                            </div>
                        </div>
                    </div>
                </div>
