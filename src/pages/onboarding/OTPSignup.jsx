...
const OTPSignup = () => {
    const { register, control, handleSubmit, formState: {errors}, watch, setValue, setError } = useForm();
    ...
    const otpSignup = (fields) => {
        setIsDisabled(true);
        if (!fields.otp || fields.otp.length !== 5 || !/^[0-9]*$/.test(fields.otp)) {
            setError("otp", {
                type: "manual",
    ...
    return (
        <>
            <div>
                ...
                <p className='xui-mt-half xui-font-sz-120 xui-font-w-600'>Create your 5 digit authentication pin</p>
            </div>
            <form className="xui-form xui-mt-2" onSubmit={handleSubmit(otpSignup)} autoComplete="off" noValidate>
                <div className="xui-form-box">
                    <label htmlFor="otp">Enter Pin:</label>
                    <input type="hidden" {...register('otp')} value={pinValue} />
                    <PinInput length={5} initialValue="" secret={false} onChange={(value) => setValue('otp', value)} type="numeric" inputMode="number" style={{padding: '10px'}} inputStyle={{borderColor: '#737373'}} inputFocusStyle={{borderColor: '#737373'}} onComplete={(value) => setValue('otp', value)} autoSelect={true} regexCriteria={/^[0-9]*$/} />
                    {errors.otp && (<p className="xui-form-error-msg">{errors.otp.message}</p>)}
                </div>
                <div className="xui-form-box">
